...
class GroupController extends BaseController 
{
    ...
    */
    public function getIndex()
    {
        ...
        if(Request::ajax())
        {
            $html = View::make('group.list-groups', array('groups' => $groups))->render();
            
            return Response::json(array('html' => $html));
        }
        
        $this->layout = View::make('group.index-group', array('groups' => $groups));
        $this->layout->title = "Groups list";
        $this->layout->breadcrumb = Config::get('breadcrumbs.groups');
    }
    ...
    */
    public function getCreate()
    {
        ...
        $this->layout = View::make('group.new-group', array('permissions' => $permissions));
        $this->layout->title = "New group";
        $this->layout->breadcrumb = Config::get('breadcrumbs.create_group');
    }
    ...
    */
    ...
    public function getShow($groupId)
    {
        try
        {
            ...
            foreach($group->getPermissions() as $permissionValue => $key)
            {
                ...
                catch(\Models\Permissions\PermissionNotFoundException $e){}
            }
            ...
            if(Request::ajax())
            {
                $html = View::make('group.list-users-group', array('group' => $group, 'users' => $users, 'candidateUsers' => $candidateUsers))->render();
                
                return Response::json(array('html' => $html));
            }
            
            $this->layout = View::make('group.show-group', array('group' => $group, 'users' => $users, 'candidateUsers' => $candidateUsers, 'permissions' => $permissions, 'ownPermissions' => $groupPermissions));
        ...
        {
            $this->layout->content = View::make('dashboard.error', array('message' => 'Sorry, group not found !'));
        }
    }
    ...
    protected function _validateGroup($permissionsValues, $groupname, &$permissions)
    {
        ...
        $validator = Validator::make(
            ...
            Config::get('rules.groups.create_name')
        );
